...
public class SameSiteCookieCsrfTokenRepository implements CsrfTokenRepository {
    ...
    public SameSiteCookieCsrfTokenRepository(Environment environment) {
    ...
    private Boolean isSecure() {
        return (this.secure != null)
                ...
                Arrays.stream(environment.getActiveProfiles())
                        .noneMatch(p -> p.equalsIgnoreCase("local"));
    }

    @Override
    public CsrfToken loadToken(HttpServletRequest request) {
        var cookie = WebUtils.getCookie(request, this.cookieName);
        if (cookie == null) {
            return null;
        }
